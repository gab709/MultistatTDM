...
System.register(['app/plugins/sdk', 'jquery', 'jquery.flot', 'lodash', 'moment', './css/multistat-panel.css!', './external/d3'], function (_export, _context) {
	...
	//TDM variables/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	var meanDataTDM;//second query results
	var valoriMeanTDM=[];//value from second query
	var initPointTDM=[];//start point lines each column
	var labelColumnTDM=[];//label for each column
	var checkTDM;
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			_export('PanelCtrl', MultistatPanelCtrl = function (_MetricsPanelCtrl) {
				...
				_createClass(MultistatPanelCtrl, [{
					...
					value: function onInitEditMode() {
						...
						this.addEditorTab('TDMOptions', 'public/plugins/michaeldmoore-multistat-panel/tdmoptions.html', 7);
					}
				}, {
					key: 'onDataReceived',
					value: function onDataReceived(data) {
						...
						if (data.length == 0) {
							...
							//Value from second query////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
							if(data.length>1){
								if(data[1].type == "table"){
									valoriMeanTDM=[];
									initPointTDM=[];
									meanDataTDM=data[1];
									for (var index = 1; index < meanDataTDM.rows[0].length; ++index) {
										valoriMeanTDM.push(meanDataTDM.rows[0][index]);
							...
							//Manage first query and add second query's value//////////////////////////////////////////////////////////////////////////////////////
							var valueFirst=[];
							var timestamp=data[0].rows[0][0];
							for (var index = 1; index < data[0].rows[0].length; ++index) {
								if(data[0].rows[0][index]==null){
									valueFirst.push(0);
								}
								else{
									valueFirst.push(data[0].rows[0][index]);
								}
							}
							//Hint text each column
							data[0].rows=[];
							data[0].columns[0].text="Elettrodomestico"
							data[0].columns[1].text="Valore"
							data[0].columns[2].text="Media"
							for (var index = 0; index < valueFirst.length; ++index) {
								data[0].rows.push([index+1,valueFirst[index],valoriMeanTDM[index]]);
							}
							this.data = data[0];
							////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					value: function onRender() {
						...
						initPointTDM=[];/////////////////////reset init point mean line
						if (this.data != null && this.data.rows != null) {
							...
							//Legend//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
							if(this.panel.TdmOption){
								this.elem.html("<svg class='" + this.className + "'  style='height:" + (this.ctrl.height-40) + "px; width:100%'>"+
								"</svg> <div><svg width=\"150\" height=\"40\">\n <rect width='20' y=\"15\" height='5' style='fill:"+this.panel.IndicatoreTDMColor+";stroke-width:1;stroke:rgb(0,0,0)' /> "+
								"<text x=\"20%\" y=\"50%\" dominant-baseline=\"middle\" text-anchor=\"start\">"+this.panel.labelIndicatoreTDM+"</text>"+
								"</svg><svg width='150' height='40'><rect width='15' y=\"10\" height='15' style='fill:"+this.panel.HighBarColor+";stroke-width:1;stroke:rgb(0,0,0)' />"+
								"<text x=\"20%\" y=\"50%\" dominant-baseline=\"middle\" text-anchor=\"start\">"+this.panel.labelIndicatoreTDM+"</text> </svg></div>");
							...
							//Check TDM option and get label for the column/////////////////////////////////////////////////////////////
							checkTDM=this.panel.TdmOption;
							labelColumnTDM=this.panel.listLabelsTDM.split(',');
							///////////////////////////////////////////////////////////////////////////
         	...
         	.attr("stroke", "#ffffff");*/
									...
									if (panel.ShowLowLimitLine) vLine(svg, lowLimitValue, panel.LowLimitLineColor);/////////////////////////////////////////////////////////////////////7
								...
								var plotGroupVertical = function plotGroupVertical(panel, svg, data, numRows, groupName, groupNameOffset, left, w, hh, dh) {
									...
									svg.append("g").selectAll("rect").data(stripedata).enter().append("rect").attr("class", "michaeldmoore-multistat-panel-row").attr("width", stripeScale.step()).attr("height", dh).attr("x", function (d, i) {//////////////////////////
										//Extract init point mean value second query//////////////////////////////////////////////////////////////////////////////////////////////////////////////
										if(checkTDM){
											initPointTDM.push(stripeScale(d));
										}
										////////////////////////////////////////////////////////////////////////////////////////////////////////////////
										return stripeScale(d);
									...
									//Draw line from second query//////////////////////////////////////////////////////////////////////////////////
									function hLineTDM(svg, value, color,inizio,fine) {
										svg.append("line").style("stroke", color).style("stroke-width", 3).attr("x1",inizio).attr("y1", valueScale(value)).attr("x2", fine).attr("y2", valueScale(value));
									}
									if(checkTDM){
										for(var index=0; index<valoriMeanTDM.length;index++){
											hLineTDM(svg, valoriMeanTDM[index], panel.IndicatoreTDMColor, initPointTDM[index],initPointTDM[index]+stripeScale.step());
										}
										...
										for (var _cc2 = 0; _cc2 < _gcols; _cc2++) {
											...
											if(checkTDM)
											{
												...
												if (_ii < this.groupedRows.length) {
													//For each column draw rectangle and add label column//////////////////////////////////////////////////////////////
													//Original:plotGroupVertical(this.panel, this.svg, this.groupedRows[_ii].values, _nn, this.groupedRows[_ii].key, _groupNameOffset, ww, _dw - GroupGap, _hh - _dh, _dh);
													plotGroupVertical(this.panel, this.svg,this.groupedRows[_ii].values, _nn, labelColumnTDM[_ii], _groupNameOffset, ww, _dw - GroupGap, _hh - _dh, _dh);
													////////////////////////////////////////////////////////////////
													ww += _dw;
												}
											}
